...
class AnpolController extends Controller
{
    ...
    {
        $ralan = RegPeriksa::select('tgl_registrasi', 'pasien.no_rkm_medis', 'pasien.nm_pasien', 'dokter.nm_dokter')
            ->join('pasien', 'pasien.no_rkm_medis', '=', 'reg_periksa.no_rkm_medis')
            ->join('dokter', 'dokter.kd_dokter', '=', 'reg_periksa.kd_dokter')
            ->where('stts', 'Belum')
            ->where('status_lanjut', 'Ralan')
    ...
    public function anpol()
    {
        $anpol = Anpol::orderBy('created_at', 'DESC')->first();
        return response()->json($anpol);
    }
}
